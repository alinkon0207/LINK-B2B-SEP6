...
export const createSubAccount = async ({ userID, firstName }) => {
  ...
  try {
    const bpResponse = await fetch(
      'https://developers.bitpowr.com/api/v1/accounts/d18be342-bd28-4559-94da-098a7f100a4c/sub-accounts/',
      options
    );
...
export const createSubAccountAddress = async ({ subId, firstName, item }) => {
  ...
  try {
    const bpResponse = await fetch(
      `https://developers.bitpowr.com/api/v1/accounts/d18be342-bd28-4559-94da-098a7f100a4c/sub-accounts/${subId}/addresses`,
      options
    );
    const response = await bpResponse.json();
    return response;
