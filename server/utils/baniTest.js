...
export const createCustomer = async ({
  ...
  const options = {
    ...
    headers: {
      ...
      Authorization: process.env.BANI_API_KEY_TEST,
      'moni-signature': process.env.BANI_MONI_KEY_TEST,
    },
    body: JSON.stringify({
  ...
  try {
    const baniResponse = await fetch(
      'https://stage.getbani.com/api/v1/comhub/add_my_customer/',
      options
    );
...
export const createCustomerAccount = async (customer_id) => {
  ...
  try {
    const baniResponse = await fetch(
      'https://stage.getbani.com/api/v1/partner/collection/bank_transfer/',
      options
    );
    const response = await baniResponse.json();
    return response;
...
export const NairaWithdraw = async ({
  ...
  try {
    const baniResponse = await fetch(
      'https://stage.getbani.com/api/v1/partner/payout/initiate_transfer/',
      options
    );
    const response = await baniResponse.json();
    return response;
...
export const NgnTransfer = async ({ amountPayout, custRef }) => {
  const options = {
    ...
    body: JSON.stringify({
      ...
      transfer_method: 'bank',
      transfer_receiver_type: 'personal',
      receiver_account_num: '7470777965',
      receiver_country_code: 'NG',
      receiver_sort_code: '000017',
      receiver_account_name: 'luke cage',
...
export const verifyAccNumber = async ({ bankCode, accNum }) => {
  ...
  try {
    const baniResponse = await fetch(
      'https://stage.getbani.com/api/v1/partner/payout/verify_bank_account/',
      options
    );
    const response = await baniResponse.json();
    return response;
